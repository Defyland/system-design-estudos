# Reviews - LLM Foundations

Revisao recall-first para a trilha paralela de fundamentos de LLM.

## Como usar

1. responda tudo em voz alta
2. puxe o `Cue Signal` antes do resto
3. olhe o gabarito logo em seguida
4. marque mentalmente cada card como:
   - `liso`
   - `hesitante`
   - `errei`

Use a mesma cadencia do repo principal:
- [Dia 00 - Pre-Sleep Flashcards](../../../../reviews/day-00-pre-sleep-flashcards.md)
- [Dia 01 - Anchor Recall](../../../../reviews/day-01-anchor-recall.md)
- [Dia 03 - Discrimination Pass](../../../../reviews/day-03-discrimination-pass.md)
- [Dia 07 - Transfer Pass](../../../../reviews/day-07-transfer-pass.md)
- [Dia 14 - Interview Compression](../../../../reviews/day-14-interview-compression.md)
- [Dia 30 - Retention Audit](../../../../reviews/day-30-retention-audit.md)

## Review Cards

<!-- curriculum:start:llm-foundations-review-cards -->
- [Card 01 - Tokens, Embeddings and Training Windows](cards/01-tokens-embeddings-and-training-windows.md)
- [Card 02 - Self-Attention, Causal Masking and Multi-Head Structure](cards/02-self-attention-causal-masking-and-multi-head-structure.md)
- [Card 03 - GPT Blocks, Residual Paths and Generation Flow](cards/03-gpt-blocks-residual-paths-and-generation-flow.md)
- [Card 04 - Pretraining Loop, Data Flow and Loss Discipline](cards/04-pretraining-loop-data-flow-and-loss-discipline.md)
- [Card 05 - Finetuning, Instruction Following and LoRA Boundaries](cards/05-finetuning-instruction-following-and-lora-boundaries.md)
- [Card 06 - From Model Internals to LLM System Design](cards/06-from-model-internals-to-llm-system-design.md)
<!-- curriculum:end:llm-foundations-review-cards -->

## Objetivo Real

O chapter ensina o mecanismo.

O review card faz voce lembrar por que esse mecanismo importa para system design de IA.
