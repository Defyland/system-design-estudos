# 09 - Backend Principles

## Por que esta area existe

Backend Principles cobre fundamentos de backend que aparecem antes de system design: HTTP, routing, contratos de API, auth, validacao, persistencia, cache, jobs, observabilidade, shutdown, seguranca e performance.

## Como estudar

Use esta area para fechar gaps praticos. Para cada card:

1. explique o conceito em voz alta;
2. desenhe o caminho do request;
3. diga o que quebra primeiro;
4. implemente ou revise um exemplo pequeno em Rails, Elixir ou Go.

## Cards

- [Backend Roadmap](./cards/backend-roadmap.md)
- [HTTP Protocol](./cards/http-protocol.md)
- [Routing](./cards/routing.md)
- [Serialization and Deserialization](./cards/serialization-deserialization.md)
- [Authentication and Authorization](./cards/auth-authz.md)
- [Validation and Transformation](./cards/validation-transformation.md)
- [Controllers, Services, Repositories, Middleware and Context](./cards/controllers-services-repositories-middleware-context.md)
- [REST API Design](./cards/rest-api-design.md)
- [Postgres Databases](./cards/postgres-databases.md)
- [Caching](./cards/caching.md)
- [Task Queues and Background Jobs](./cards/task-queues-background-jobs.md)
- [Transactional Emails](./cards/transactional-emails.md)
- [Full Text Search and Elasticsearch](./cards/full-text-search-elasticsearch.md)
- [Error Handling and Fault Tolerance](./cards/error-handling-fault-tolerance.md)
- [Configuration Management](./cards/configuration-management.md)
- [Logging, Monitoring and Observability](./cards/logging-monitoring-observability.md)
- [Graceful Shutdown](./cards/graceful-shutdown.md)
- [Backend Security](./cards/backend-security.md)
- [Object Storage and Large Files](./cards/object-storage-large-files.md)
- [Realtime Backend Systems](./cards/realtime-backend-systems.md)
- [Testing and Code Quality](./cards/testing-code-quality.md)
- [Twelve-Factor App](./cards/twelve-factor-app.md)
- [OpenAPI Standards](./cards/openapi-standards.md)
- [Webhooks](./cards/webhooks.md)
- [DevOps for Backend Engineers](./cards/devops-for-backend-engineers.md)
- [Scaling and Performance](./cards/scaling-performance.md)
- [Concurrency and Parallelism](./cards/concurrency-parallelism.md)
- [Engineering Blog Reading Map](./cards/engineering-blog-reading-map.md)

## Fonte

Baseado na playlist [Backend from first principles](https://www.youtube.com/playlist?list=PLui3EUkuMTPgZcV0QhQrOcwMPcBCcd_Q1), de Sriniously. Os cards resumem o uso pratico dos temas e evitam depender de transcricao longa.
