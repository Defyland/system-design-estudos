# Chapter 04 - Event Backbone, Partitions and Consumer Scale
...
- `Study Order`: `04/14` - `Fase 1 - Base forte`
- `Caso real principal`: [LinkedIn - Kafka Backbone](../real-world-cases/03-async-workflows-and-payments/linkedin-kafka-backbone/README.md)
- `Area principal`: [03 - Filas e Consistencia](../areas/03-filas-e-consistencia/README.md)
- `Area secundaria`: [01 - Metodo e Entrevistas](../areas/01-metodo-e-entrevistas/README.md)
- `Notes principais`: [Filas e Consistencia](../areas/03-filas-e-consistencia/notes.md), [Metodo e Entrevistas](../areas/01-metodo-e-entrevistas/notes.md)
...
- `Requirement Less Dumb`: voce realmente precisa de varios consumidores independentes e replay, ou so quer fazer uma tarefa assincrona simples?
- `Delete`: elimine eventos que so repetem chamada interna ou refletem acoplamento mal resolvido entre modulos.
- `Simplify`: fila simples com outbox bem feita ganha de backbone completo quando o problema ainda e local e linear.
- `Accelerate`: introduza schema, lag e replay pequeno cedo para aprender onde o throughput e a ordem realmente apertam.
- `Automate Last`: rebalancing, auto-scale de consumers e governanca pesada entram depois que o contrato do evento ja parou de mudar.

### Fixacao Relampago: Design Pass

- `Pergunta`: qual e a primeira pergunta antes de escolher Kafka ou equivalente?
- `Resposta com as suas palavras`: eu preciso saber se o sistema pede consumidores independentes com replay, nao apenas async por si so.
- `Resposta ruim que parece boa`: "toda comunicacao assincrona merece backbone de eventos".
- `Troque por isto`: primeiro eu provo a necessidade de fanout e replay; sem isso, uma fila menor costuma ser o desenho certo.
...
- [Prev](./chapter-03-idempotent-writes-under-ambiguous-failure.md)
- [Index](./README.md)
- [Next](./chapter-05-durable-workflows-retries-and-compensation.md)
