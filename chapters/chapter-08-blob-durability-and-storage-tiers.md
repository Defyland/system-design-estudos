# Chapter 08 - Blob Durability and Storage Tiers
...
- `Study Order`: `08/14` - `Fase 2 - Caminhos criticos e dados especializados`
- `Caso real principal`: [Dropbox - Magic Pocket Blob Store](../real-world-cases/02-data-storage-and-search/dropbox-magic-pocket-blob-store/README.md)
- `Area principal`: [02 - Dados e Armazenamento](../areas/02-dados-e-armazenamento/README.md)
- `Notes principais`: [Dados e Armazenamento](../areas/02-dados-e-armazenamento/notes.md)
...
- `Requirement Less Dumb`: voce precisa guardar bytes em escala com durabilidade e custo controlado, ou so anexos modestos que ainda cabem em desenho mais simples?
- `Delete`: remova copia inutil, thumbnail sem dono e uso indevido do banco relacional para guardar blob bruto.
- `Simplify`: separe metadata e autorizacao no core transacional; deixe o byte grande no object store.
- `Accelerate`: faca restore, checksum e troca de tier cedo para aprender onde o lifecycle realmente machuca.
- `Automate Last`: movimentacao automatica entre tiers e politicas agressivas so entram quando padrao de acesso esta confirmado.

### Fixacao Relampago: Design Pass

- `Pergunta`: qual separacao conceitual geralmente limpa esse problema primeiro?
- `Resposta com as suas palavras`: eu separo metadata de blob; o banco continua dono do contexto e o object store vira dono do byte pesado.
- `Resposta ruim que parece boa`: "se eu ja tenho Postgres, guardo tudo la mesmo".
- `Troque por isto`: primeiro eu separo contexto de armazenamento bruto; isso reduz custo e simplifica evolucao.
...
- [Prev](./chapter-07-critical-checkout-flows-and-auth-boundaries.md)
- [Index](./README.md)
- [Next](./chapter-09-search-indexing-and-permission-aware-retrieval.md)
