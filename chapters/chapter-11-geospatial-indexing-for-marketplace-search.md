# Chapter 11 - Geospatial Indexing for Marketplace Search
...
- `Study Order`: `11/14` - `Fase 2 - Caminhos criticos e dados especializados`
- `Caso real principal`: [Uber - H3 Geospatial Marketplace](../real-world-cases/02-data-storage-and-search/uber-h3-geospatial-marketplace/README.md)
...
- `Notes principais`: [Dados e Armazenamento](../areas/02-dados-e-armazenamento/notes.md), [Metodo e Entrevistas](../areas/01-metodo-e-entrevistas/notes.md)
...
- `Requirement Less Dumb`: o produto precisa procurar em inventario enorme com proximidade frequente, ou uma consulta de distancia no banco ainda basta?
- `Delete`: corte resolucao excessiva, geometria sofisticada e bucket desnecessario antes de provar o ganho.
- `Simplify`: comece por bucket aproximado mais distancia exata, ou so PostGIS, conforme volume real de candidatos.
- `Accelerate`: teste cidades quentes, borda de celula e atualizacao de localizacao cedo para achar o verdadeiro custo.
- `Automate Last`: ajuste dinamico de resolucao e redistribuicao automatica so entram quando o padrao espacial esta claro.

### Fixacao Relampago: Design Pass

- `Pergunta`: o que voce pergunta antes de dizer que H3 entrou no desenho?
- `Resposta com as suas palavras`: eu preciso saber se o problema e volume de candidatos e vizinhanca repetitiva, nao apenas coordenada no banco.
- `Resposta ruim que parece boa`: "tem mapa, entao precisa de H3".
- `Troque por isto`: primeiro eu provo que bucket espacial melhora a busca; sem isso, distancia direta pode bastar.
...
- [Prev](./chapter-10-cdn-placement-dns-and-cache-invalidation.md)
- [Index](./README.md)
- [Next](./chapter-12-distributed-ids-and-ordering-guarantees.md)
